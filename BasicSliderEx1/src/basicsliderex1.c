...
static void slider2_changed_cb(void *data, Evas_Object *obj, void *event_info) {
	appdata_s *ad = data;
	char buf[64];
	double value = elm_slider_value_get(obj);
...
static void create_base_gui(appdata_s *ad) {
	...
	evas_object_show(box);
	{
		...
		evas_object_smart_callback_add(slider, "changed", slider2_changed_cb,
				ad);
		my_box_pack(box, slider, 1.0, 0.1, -1.0, 0.0);
	}
