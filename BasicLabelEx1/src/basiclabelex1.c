...
//			my_box_pack(box, ad->label, 1.0, 0.0, 0.0, 0.5);
//		}
//	}

	{
		...
		evas_object_resize(ad->label, 400, 80);
		evas_object_show(ad->label);

		Evas_Object *label = elm_label_add(ad->conform);
		elm_object_text_set(label,
				_("<font_size=20><align=center>fontsize is set to 20</align></font_size>"));
		...
		elm_object_text_set(label,
				_("<color=#FF4500FF><align=right>font color orange red</align></color>"));
		...
		label = elm_label_add(ad->conform);
		elm_object_text_set(label,
				_("<font_size=24>If the string length exceeds the width</font_size>"));
		...
		elm_object_text_set(label,
				_("<font_size=20><align=left>Once upon a time there lived a young prince.<br>Mountan is mountain, water is water. </align></font_size>"));
