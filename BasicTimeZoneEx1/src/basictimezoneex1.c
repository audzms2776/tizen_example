...
static i18n_ucalendar_h create_time(char *tzid) {
	...
	if (ret != 0) {
		dlog_print(DLOG_ERROR, LOG_TAG,
				"i18n_ucalendar_create() failed with err = %d", ret);
		return NULL;
	}
	return ucal;
}
...
static void update(appdata_s *ad) {
	...
	char buf[256];
	int diff;
	/* Current time */i18n_ucalendar_get_now(&udate);
...
static void create_base_gui(appdata_s *ad) {
	...
	evas_object_show(table);
	{
		...
		table_pack(table, o, 0, 0, 1, 1);

		ad->label1 = elm_label_add(table);
		table_pack(table, ad->label1, 1, 0, 1, 1);
