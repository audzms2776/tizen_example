...
static void parse_json(appdata_s *ad) {
...
//		sprintf(buf, "%s - %d", buf, json_array_get_int_element(temp_array, 0));
	}

	const char* data2 =
			"{'time': '03:53:25 AM', 'millisec_epoch': 1362196405309, 'date': '03-02-2013'}";
	...
	if (json_parser_load_from_data(parser, data2, strlen(data2), NULL)) {
		...
		long long epoch_data = json_object_get_int_member(obj,
				"millisec_epoch");
		char* date_data = json_object_get_string_member(obj, "date");

		sprintf(buf,
				"%s <br/><br/> time - %s <br/> epoch - %lld <br/> date - %s",
				buf, time_data, epoch_data, date_data);
	}
